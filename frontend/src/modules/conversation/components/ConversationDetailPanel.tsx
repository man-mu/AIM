...
function conversationInitial(avatar: string, name: string): string {
  return (avatar.trim() || name.trim()).slice(0, 1).toUpperCase() || '?';
}
    ...
    <section aria-label="会话详情" className="space-y-6">
      <div className="border-b border-black/[0.08] pb-5">
        <span className="grid size-12 place-items-center rounded-full bg-[#dbe8f7] text-lg font-semibold text-[#24527a]" aria-hidden>
          {conversationInitial(activeConversation.avatar, activeConversation.name)}
        </span>
      ...
      {isGroup ? (
        <div>
          ...
          <p className="mt-2 min-w-0 break-words whitespace-pre-wrap text-sm leading-6 text-[#424245]">
